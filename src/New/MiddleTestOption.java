...
//		4. score의 모든 요소에 50~100 사이의 임의의 값(정수)을 저장한다. (5점)
        ...
        do {
            if (switch1 && switch2) {
                int addScore[][] = new int[names.length][subjects.length];
                //반복전 스코어[][]에 새로만들 배열을 넣자!
                //배열을 생성했으니까 배열을 복사하자
                //맞는거 같은데 여기서 오류뜸
                /**
                 * 배열 복사
                 */
                for (int i = 0; i < names.length - 1; i++) { // 여기서의 for문은 score 배열의 값을 복사하기위함.
                    for (int j = 0; j < subjects.length - 1; j++) {
                        addScore[i][j] = score[i][j]; //추가된 학생의 점수가 들어갈 자리는 마련햇지만 값이 들어가지 않은 상태//
                    }
                }
                //추가된 학생의 점수가 랜덤값으로 나와야됨.
                for (int i = 1; i < 2; i++) {
                    if (switch1) {
                        for (int j = 0; j < subjects.length; j++) {
                            addScore[names.length - i][j] = rd.nextInt(51) + 50;
                        }
                    } else if (switch2) {
                        for (int j = 0; j < names.length; j++) {
                            addScore[j][subjects.length - i] = rd.nextInt(51) + 50;
                        }
                    }
                    score = addScore;
                }
...
//		새로 추가된 학생의 점수를 포함하여 계산된 총 출력을 다시 보여줍니다.
                ...
                if (switch1) {
                    ...
                    names = addStu;
                    //새로 생성한 배열의 마지막 값에 값 넣어주기 (names는 고정 과목만 바뀌고 있지)
                }

                if (switch2) {
                    String[] addSub = new String[subjects.length + 1];
                     ...
                     * 추가된 데이터 배열에 넣어주기
                     */
                    addSub[addSub.length - 1] = plusStu;
                    subjects = addSub;
                }
